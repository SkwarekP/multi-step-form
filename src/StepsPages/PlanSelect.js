...
function PlanSelect() {
    ...
    useEffect(() => {
        const latestState = isClickedState.map(el => (el.id === selectedPlan.id ?
            {
                type: selectedPlan.type,
                id: selectedPlan.id,
                isClicked: selectedPlan.isClicked,
                price: selectedPlan.price,
                monthly: selectedPlan.monthly
            }
            : el))
        if (!selectedPlan.monthly) {
            setSwitcherToggle(true);
        }
        setIsClickedState(latestState);
    ...
    return (
        <Layout currentUrl={"http://localhost:3000/selectplan"} onSend={sendDataHandler}>
            ...
            <div
                className={`${isClickedState[0].isClicked ? classes.plan__card + " " + classes.plan__card__chosen : classes.plan__card}`}
                onClick={(e) => {
                    e.preventDefault();
                    if (switcherToggle) {
                ...
                <div className={classes.plan__text}>
                    ...
                    {switcherToggle &&
                        <p style={{color: "hsl(213, 96%, 18%)", fontSize: ".85rem", marginTop: ".5rem"}}>2 months
                            free</p>}
                </div>
            </div>
            <div
                className={`${isClickedState[1].isClicked ? classes.plan__card + " " + classes.plan__card__chosen : classes.plan__card}`}
                id="plancard2"
                onClick={(e) => {
                    e.preventDefault();
                    if (switcherToggle) {
            ...
            <div
                className={`${isClickedState[2].isClicked ? classes.plan__card + " " + classes.plan__card__chosen : classes.plan__card}`}
                id="plancard3"
                onClick={(e) => {
                    e.preventDefault();
                    if (switcherToggle) {
